...
class InvocableHandlerMethodK(bean: Any, method: Method) : HandlerMethod(bean, method), ProcessInvokeService {

    private val parameterNameDiscoverer: ParameterNameDiscoverer = DefaultParameterNameDiscoverer()

    companion object {
        private val jackson2FlatMapper = Jackson2FlatMapperK();
    }

    override fun isReturnVoid(): Boolean {
        return isVoid;
    }

    override fun invokeProcess(providedArgs: Map<String, Any>?): Any? {
        return invoke(jackson2FlatMapper.fromFlatMapper(providedArgs));
    }

    @Throws(Exception::class)
    private operator fun invoke(providedArgs: Map<String, Any>?): Any {
        val args = getMethodArgumentValues(providedArgs);
        //TODO:
        return bridgedMethod.invoke(bean, args);
    }

    @Throws(Exception::class)
    private fun getMethodArgumentValues(providedArgs: Map<String, Any>?): Array<Any?> {
